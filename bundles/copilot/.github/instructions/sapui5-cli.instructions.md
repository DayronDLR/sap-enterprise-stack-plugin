---
applyTo: **/webapp/**/*.js,**/webapp/**/*.xml,**/manifest.json,**/*.view.xml
...
Esta skill integra con el servidor oficial `@ui5/mcp-server`, que provee acceso en vivo a la API de UI5, scaffolding, linting y validación de proyectos directamente desde el agente.

**Herramientas disponibles** (`mcp__sap-ui5__*`):

| Herramienta | Descripción |
|-------------|-------------|
| `get_api_reference` | Obtiene documentación de la API de UI5 con firmas y estado de deprecación |
| `get_guidelines` | Accede a guías de mejores prácticas de desarrollo UI5 |
| `get_version_info` | Recupera información de versión del framework UI5 |
| `get_project_info` | Extrae metadata y configuración de un proyecto UI5 existente |
| `create_ui5_app` | Genera scaffolding de nueva app UI5 (JS/TS, standalone o integrada con CAP) |
| `create_integration_card` | Genera scaffolding de una nueva UI Integration Card |
| `run_ui5_linter` | Analiza código UI5 en busca de APIs deprecadas y otros problemas |
| `run_manifest_validation` | Valida el manifest.json contra el schema de UI5 |
| `get_typescript_conversion_guidelines` | Guía para convertir apps/controles de JavaScript a TypeScript |
| `get_integration_cards_guidelines` | Guías de mejores prácticas para UI Integration Cards |

**Cuándo usar cada herramienta**:

- Consultas sobre la API de controles → `get_api_reference`
- Crear proyecto nuevo → `create_ui5_app`
- Verificar configuración de proyecto existente → `get_project_info`
- Revisar calidad del código → `run_ui5_linter` + `run_manifest_validation`
- Migración a TypeScript → `get_typescript_conversion_guidelines`

**Integración automática**: Cuando esta skill esté activa, usa `get_api_reference` por defecto para preguntas sobre controles UI5 y `run_ui5_linter` al revisar código existente.
...
- **Fiori Elements generation**: `sap-fiori-tools` skill (canonical for Fiori Elements, Page Editor, annotations)
- **SAPUI5 FreeStyle patterns**: `sapui5-freestyle` skill (canonical for custom views, controllers, routing)
- **CAP backend integration**: `sap-cap-capire` skill (canonical for CDS models, OData services)
