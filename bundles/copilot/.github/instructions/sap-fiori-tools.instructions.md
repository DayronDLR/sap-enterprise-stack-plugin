...
### @UI.dataPoint with Criticality

```
// @UI.dataPoint with Criticality
@UI.lineItem: [{ position: 10, criticality: 'StatusCriticality' }]
@UI.dataPoint: { criticality: 'StatusCriticality' }
// StatusCriticality values: 0=neutral, 1=negative, 2=critical, 3=positive
```
...
Esta skill integra con el servidor oficial `@sap-ux/fiori-mcp-server`, que provee acceso en vivo a documentación, escaneo de proyectos y generación/modificación automatizada de apps Fiori.

**Herramientas disponibles** (`mcp__sap-fiori-tools__*`):

| Herramienta | Descripción |
|-------------|-------------|
| `search_docs` | Busca en documentación de SAP Fiori Elements, Annotations, UI5 y Fiori Tools |
| `list_fiori_apps` | Escanea un directorio e identifica apps Fiori existentes que pueden modificarse |
| `list_functionalities` | Lista funcionalidades soportadas para crear o modificar apps Fiori |
| `get_functionality_details` | Obtiene parámetros requeridos y specs detalladas de una funcionalidad específica |
| `execute_functionality` | Implementa una funcionalidad para crear o modificar una app Fiori |

**Cuándo usar cada herramienta**:

- Antes de responder preguntas sobre Fiori Elements o anotaciones → `search_docs`
- Para descubrir apps existentes en un workspace → `list_fiori_apps`
- Para implementar o modificar funcionalidades → flujo: `list_functionalities` → `get_functionality_details` → `execute_functionality`

**Integración automática**: Cuando esta skill esté activa, usa `search_docs` por defecto para consultas de documentación y el flujo completo para implementaciones.
...
- **CAP integration**: `sap-cap-capire` skill (canonical for CDS models, service handlers)
- **UI5 CLI tooling**: `sapui5-cli` skill (canonical for ui5 build, serve, add commands)
- **SAPUI5 FreeStyle development**: `sapui5-freestyle` skill (canonical for custom views, controllers, routing)
