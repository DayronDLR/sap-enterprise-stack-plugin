...
- **Versión del framework**: fijar versión LTS explícita en `manifest.json` (LTS vigente **1.136**; mínimo soportado ~1.120) — NUNCA `latest`; validar con `mcp_sap_ui5_get_version_info` (ver `rules/SAPUI5-CAP-Integration.md`)
- **TypeScript-first** en proyectos nuevos: tipos UI5 (`@sapui5/types`), controllers/formatters en `.ts`, build con UI5 Tooling — ver `mcp_sap_ui5_get_typescript_conversion_guidelines`
