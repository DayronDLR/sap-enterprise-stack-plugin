...
Call `mcp_sap_ui5_get_guidelines` first. Always. Apply the output throughout the entire task.
...
```text
1. mcp_sap_ui5_get_version_info { frameworkName: "SAPUI5" }
2. mcp_sap_ui5_create_ui5_app { appNamespace, basePath, typescript: true, framework: "SAPUI5", runNpmInstall: true }
3. mcp_sap_fiori_tools_list_functionality { appPath }
   → mcp_sap_fiori_tools_get_functionality_details { appPath, functionalityId }
   → mcp_sap_fiori_tools_execute_functionality { appPath, functionalityId, parameters }
```
...
When unsure about a UI5 API: `mcp_sap_ui5_get_api_reference` or `mcp_sap_fiori_tools_search_docs`.
...
```text
mcp_sap_ui5_run_ui5_linter { projectDir }          ← fix all findings
mcp_sap_ui5_run_manifest_validation { projectDir }
```
