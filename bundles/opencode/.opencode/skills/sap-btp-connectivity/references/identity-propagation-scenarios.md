...
Required for principal propagation to work end-to-end:

### Steps

1. **BTP Cockpit** > Subaccount > Security > Trust Configuration
2. Add custom IDP (SAP Identity Authentication or corporate IDP)
3. Exchange SAML metadata:
   - Download BTP subaccount metadata (SP metadata)
   - Upload to IDP as relying party
   - Download IDP metadata (IdP metadata)
   - Upload to BTP trust configuration
4. Configure attribute mapping (email, user ID, groups)
5. Test SSO login via BTP Launchpad
...
| Error | Cause | Fix |
|-------|-------|-----|
| 403 Forbidden | User mapping failed | Check IDP attribute mapping |
| SAML assertion invalid | Trust not established | Re-exchange SAML metadata |
| No principal propagated | Missing trust config | Add IDP in BTP cockpit |
