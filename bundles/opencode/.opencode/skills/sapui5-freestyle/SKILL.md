...
Call `mcp.sap-ui5.get_guidelines` first. Always. Apply the output throughout the entire task.
...
```text
1. mcp.sap-ui5.get_version_info { frameworkName: "SAPUI5" }
2. mcp.sap-ui5.create_ui5_app { appNamespace, basePath, typescript: true, framework: "SAPUI5", runNpmInstall: true }
3. mcp.sap-fiori-tools.list_functionality { appPath }
   → mcp.sap-fiori-tools.get_functionality_details { appPath, functionalityId }
   → mcp.sap-fiori-tools.execute_functionality { appPath, functionalityId, parameters }
```
...
When unsure about a UI5 API: `mcp.sap-ui5.get_api_reference` or `mcp.sap-fiori-tools.search_docs`.
...
```text
mcp.sap-ui5.run_ui5_linter { projectDir }          ← fix all findings
mcp.sap-ui5.run_manifest_validation { projectDir }
```
