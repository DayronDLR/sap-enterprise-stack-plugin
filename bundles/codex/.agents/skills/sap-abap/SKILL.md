...
### RAP Draft Enablement in BDEF

```abap
// RAP Draft enablement in BDEF
managed with draft implementation in class zbp_i_order unique;
draft action Resume;
draft action Edit;
draft action Activate optimized;
draft action Discard;
draft determine action Prepare;
// Draft table: ZDRAFT_ORDER (auto-generated or manual)
```

### RAP Unit Test with cl_abap_behv_test_environment

```abap
// RAP Unit Test with cl_abap_behv_test_environment
CLASS ltcl_order FOR TESTING DURATION SHORT RISK LEVEL HARMLESS.
  CLASS-DATA: environment TYPE REF TO cl_abap_behv_test_environment.
...
CLASS ltcl_order IMPLEMENTATION.
  METHOD class_setup.
    environment = cl_abap_behv_test_environment=>create( i_for_entity = 'ZI_ORDER' ).
  ENDMETHOD.
  " ... test methods using MODIFY ENTITIES + COMMIT ENTITIES
ENDCLASS.
```
...
- **CDS view development**: `sap-abap-cds` skill (canonical for ABAP CDS annotations, DCL, access control)
- **SQLScript procedures**: `sap-sqlscript` skill (canonical for HANA stored procedures, functions)
- **BTP connectivity**: `sap-btp-connectivity` skill (canonical for RFC destinations, Cloud Connector)
