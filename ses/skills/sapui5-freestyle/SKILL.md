...
Call `mcp__sap_ui5__get_guidelines` first. Always. Apply the output throughout the entire task.
...
```text
1. mcp__sap_ui5__get_version_info { frameworkName: "SAPUI5" }
2. mcp__sap_ui5__create_ui5_app { appNamespace, basePath, typescript: true, framework: "SAPUI5", runNpmInstall: true }
3. mcp__sap_fiori_tools__list_functionality { appPath }
   → mcp__sap_fiori_tools__get_functionality_details { appPath, functionalityId }
   → mcp__sap_fiori_tools__execute_functionality { appPath, functionalityId, parameters }
```
...
When unsure about a UI5 API: `mcp__sap_ui5__get_api_reference` or `mcp__sap_fiori_tools__search_docs`.
...
```text
mcp__sap_ui5__run_ui5_linter { projectDir }          ← fix all findings
mcp__sap_ui5__run_manifest_validation { projectDir }
```
