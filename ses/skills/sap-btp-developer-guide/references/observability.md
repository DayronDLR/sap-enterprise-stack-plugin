...
**Based on**: OpenSearch (OpenSearch Dashboards for visualization)

**Deployment**: Instance-based service

### Application Logging with @sap/logging
...
srv.on('READ', 'Orders', async (req) => {
  const logger = appContext.createLogContext().getLogger('/orders');
  logger.info('Reading orders for user %s', req.user.id);
  // ...
});
```
